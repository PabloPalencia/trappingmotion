...
# devtools::install_github("PabloPalencia/trappingmotion") # run this line once to install the package
...
Three different movement behaviours have been identified in this population.
...
RedDeerdata$T_0_1 <- (as.numeric(strptime(RedDeerdata$Time, format="%H:%M:%S") - as.POSIXct(format(Sys.Date())), units="secs")/86400)
...
Lets create a dataframe that includes the speeds results

```{r echo=FALSE}
speed_data <- data.frame(speeds=res[1,], speed_se=res[2,], n_seq=c(length(which(behav_class$behaviour == "1" )), length(which(behav_class$behaviour == "2" )), length(which(behav_class$behaviour == "3" ))))
...
head(speed_data)
```
Run the `dayrange` function to estimate day range (i.e. average daily distance travelled of this red deer population)

```{r}
dayrange(act, act_se, speed_data)
```

The day range of this population is 5.07km per day
